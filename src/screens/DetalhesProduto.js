...
const DetalhesProduto = () => {
        ...
        if (productData && productData.tamanhos) {
          if (productData.tamanhos.tipo === "calcado") {
            ...
            const available = all.filter(
              (s) =>
                !(productData.tamanhos.indisponiveis || []).includes(Number(s))
            );
            if (available.length) setSelectedSize(available[0]);
          } else {
            const op = productData.tamanhos.opcoes || [];
            const available = op.filter(
              (s) => !(productData.tamanhos.indisponiveis || []).includes(s)
            );
            if (available.length) setSelectedSize(available[0]);
          }
        }
  ...
  const loadComentarios = async () => {
    try {
      ...
      const comentarios =
        result.Items?.filter((i) => i.productId?.S === String(productId)) || [];

      if (comentarios.length > 0) {
        const notas = comentarios.map((i) => Number(i.rating?.N || 0));
  ...
  const unmarshallProduct = (dynamoItem) => {
        ...
        cores = dynamoItem.cores.L.map((it) => {
          if (it.M) {
            ...
            Object.keys(it.M).forEach((k) => {
              ...
              plain[k] =
                v.S ||
                (v.N ? Number(v.N) : undefined) ||
                (v.L ? v.L.map((x) => x.S) : undefined);
            });
            return plain;
          }
          return null;
    ...
    const tamanhos =
      tamanhosAttr ||
      (dynamoItem.tamanhos?.L ? dynamoItem.tamanhos.L.map((x) => x.S) : []);
    ...
    return {
      ...
      tamanhos: tamanhos,
      avaliacao: dynamoItem.avaliacao?.N
        ? parseFloat(dynamoItem.avaliacao.N)
        : 0,
      cores: cores,
      // caso exista imagens flat
      imagensFlat: dynamoItem.imagens?.S
        ? JSON.parse(dynamoItem.imagens.S)
        : [],
    };
  };
  ...
  const sizesToShow = (() => {
    ...
    if (product.tamanhos.tipo === "calcado") {
      ...
      return arr.map((s) => ({
        label: s,
        disponivel: !indispon.includes(Number(s)),
      }));
    }
    // roupa
    if (product.tamanhos.opcoes) {
      const indispon = product.tamanhos.indisponiveis || [];
      return product.tamanhos.opcoes.map((sz) => ({
        label: sz,
        disponivel: !indispon.includes(sz),
      }));
    }
    ...
    <ScrollView style={styles.container}>
          ...
          {sizesToShow.map((item) => (
            ...
            >
              <Text
                style={[
                  styles.sizeText,
                  item.label === selectedSize && styles.sizeTextSelected,
                ]}
              >
                {item.label}
              </Text>
      ...
      <View style={styles.reviewsSection}>
        ...
        <View style={styles.ratingRow}>
          <Text style={styles.ratingValue}>{ratingMedia.toFixed(1)}</Text>
          {renderStars(ratingMedia)}
        </View>
        <TouchableOpacity
          onPress={() => navigation.navigate("Comentarios", { productId })}
        >
